...
<div class="panel panel-default">
	...
	<div class="panel-body">
		<table class="table userIndexTable table-bordered">
			...
			<?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lis): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($lis["username"]); ?></td>
					<td><?php echo floor($lis['ymoney'])/100;?>元</td>
					<td><?php echo floor($lis['money'])/100;?>元</td>
					<td>
						<?php if(($lis["account_type"]) == "1"): ?>微信<?php endif; ?>
						<?php if(($lis["account_type"]) == "2"): ?>支付宝<?php endif; ?>
						<?php if(($lis["account_type"]) == "3"): ?>银行卡<?php endif; ?>
					</td>
					<td><?php echo ($lis["account"]); ?></td>
					<td><?php echo C('SET_REMIND');?>%</td>
					<td><?php echo floor($lis['tmoney'])/100;?>元</td>
					...
					<?php } ?>
					<td>
						<?php if(($lis["status"]) == "0"): echo BA('userfinance/refuse_cause',array("log_id"=>$lis["log_id"]),'拒绝原因','load','',600,280); endif; ?>
						<?php if(($lis["status"]) == "2"): ?><code>审核通过</code><?php endif; ?>
						<?php if(($lis["status"]) == "1"): echo BA('userfinance/ok',array("log_id"=>$lis["log_id"]),'通过','act','');?>
							| <?php echo BA('userfinance/refuse',array("log_id"=>$lis["log_id"]),'拒绝','load','',600,280); endif; ?>
					</td>
