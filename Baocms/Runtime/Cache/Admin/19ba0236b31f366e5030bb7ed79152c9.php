...
<ul class="breadcrumb">
    ...
    <li>
        <a href="<?php echo U('index');?>">订单流水</a><span class="divider"></span>
    </li>
</ul>
	...
	<div class="panel-heading">
		...
		<form action="" class="form-inline" method="POST">
			...
			<div class="form-group">
				...
				<input type="text" name="shop_name" id="" value="<?php echo ($shop_name); ?>" class="form-control">
			</div>
	...
	<div class="panel-body">
		<table class="table userIndexTable table-bordered">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lis): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($lis["shop_name"]); ?>【<?php echo ($lis["tel"]); ?>】</td>
					<td><?php echo (date("Y-m-d H:i:s",$lis["create_time"])); ?></td>
					<td><?php echo floor($lis['smoney'])/100;?>元</td>
	                <td><?php echo floor($lis['money'])/100;?>元</td>
	                <td><?php echo floor($lis['p_price'])/100;?>元</td>
	                <td><?php echo floor($lis['s_price'])/100;?>元</td>
	                <td>
