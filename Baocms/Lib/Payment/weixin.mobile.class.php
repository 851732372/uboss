...
class weixin {
    ...
    public function getCode($logs, $payment) {
        ...
        //①、获取用户openid
        $tools = new JsApiPay();
        $openId = session('openid') ? session('openid') : $tools->GetOpenid($logs['logs_id']);
//       echo $openId;die;
        ...
        $input->SetNotify_url('http://m.uboss.net.cn/' . U( 'api/order/respond', array('code' => 'weixin')));
		...
		WeixinJSBridge.invoke(
			...
			function(res){
                            if(res.err_msg ==\'get_brand_wcpay_request:ok\'){ 
                                location.href="'.U('api/order/yes',array('log_id'=>$logs['logs_id'])).'";// U(\'mobile/payment/yes\',array(\'log_id\'=>$logs[\'logs_id\'])).\'";
                            }
			}
		);
	}
