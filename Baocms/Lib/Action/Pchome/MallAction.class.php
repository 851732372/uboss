...
class MallAction extends CommonAction {
    ...
    public function main() {
        ...
        if (!$shops = $cache->get('lists')) {
            if (!empty($cates)) {
                ...
                foreach ($cates as $k => $val) {
                    if (!empty($val['cate_name']) && !empty($val['cate_id'])) {
                        $map['cate_id'] = $val['cate_id'];
                    }
                    $lists[$k] = D('WeidianDetails')->where($map)->limit(0, 6)->select();
                }
        ...
        if(!empty($shop_ids)){
            $this->assign('wshop',D('Shop')->itemsByIds($shop_ids));
        }
    ...
    public function cartadd() {
        ...
        if (!$detail = D('Goods')->find($goods_id)) {
            $this->niuError('改商品不存在');
        }
        if ($detail['closed'] != 0 || $detail['audit'] != 1) {
            $this->niuError('该商品不存在');
        }
        if ($detail['end_date'] < TODAY) {
            $this->niuError('该商品已经过期，暂时不能购买');
        }
        $goods = cookie('goods');
        ...
        cookie('goods', $goods, 604800);
        $this->niuSuccess('加入购物车成功！', U('mall/detail', array('goods_id' => $goods_id)));
    }
    ...
    public function order_change() {
        ...
        if (empty($goods)) {
            $this->niuError('很抱歉，您提交的产品暂时不能购买！');
        }
        $tprice = 0;
        $ip = get_client_ip();
        ...
        if($count == count($goods)){
            if(false !== D('Order')->save(array('order_id'=>$order_id,'total_price'=>$tprice,'update_time'=>NOW_TIME,'update_ip'=>$ip))){
                $this->niuJump(array('status' => 'success', 'msg' => '成功修改订单，正在跳转到支付页面','url'=>U('mall/pay', array('order_id' => $order_id))));
            }else{
                $this->niuJump(array('status' => 'error', 'msg' => '修改订单失败'));
            }
        }
    }
    ...
    public function change_addr() {
        if (IS_AJAX) {
            ...
            if (false !== D('Order')->save($data)) {
                ...
                $this->baoJump(array('status' => 'success', 'msg' => '更换成功', 'res' => $addr_array));
            } else {
                $this->baoJump(array('status' => 'error', 'msg' => '更换失败'));
            }
        }
    }
    ...
    public function paycode2() { //这里是因为原来的是按订单付，这里是合并付款逻辑部分 
        ...
        if ($code == 'wait') { //如果是货到付款
            ...
            $this->niuSuccess('恭喜您下单成功AAA！', U('member/goods'));
        } else {
            $payment = D('Payment')->checkPayment($code);
            if (empty($payment)) {
                $this->niuError('该支付方式不存在');
            }
            $detail['code'] = $code;
            D('Paymentlogs')->save($detail);
            $this->niuJump(U('mall/combine', array('log_id' => $detail['log_id'])));
        }
    }
    ...
    public function pay2() {
        ...
        if ($code == 'wait') { //如果是货到付款
            ...
            $this->niuSuccess('恭喜您下单成功！', U('member/order/goods'));
        } else {
            $payment = D('Payment')->checkPayment($code);
            if (empty($payment)) {
                $this->niuError('该支付方式不存在');
	 ...
	 public function addressadd() {
		...
		if ($this->isPost()){
			...
			if ($obj->add($data)) {
				...
				$this->niuSuccess('新增收货地址成功', $backurl);
			}
			$this->niuError('操作失败！');
	...
	private function addressCheck() {
		...
		if (empty($data['mobile'])) {
			$this->niuError('手机号码不能为空');
		}
		if (!isMobile($data['mobile'])) {
			$this->niuError('手机号码格式不正确');
		}
		$data['addr'] = htmlspecialchars($data['addr']);
		if (empty($data['addr'])) {
			$this->niuError('具体地址不能为空');
		}
		return $data;
	}
    ...
    public function addressdel() {
		...
		if (empty($addr_id)) {
			$this->niuError('收货地址不存在');
		}
		if (!$detail = D('Useraddr')->find($addr_id)) {
			$this->niuError('收货地址不存在');
		}
		if ($detail['user_id'] != $this->uid) {
			$this->niuError('请不要操作别人的收货地址');
		}
		$obj = D('Useraddr');
		$obj->save(array('addr_id' => $addr_id, 'closed' => 1));
		$this->niuSuccess('删除成功！', U('mall/pay',array('order_id'=>$order_id)));
	}
