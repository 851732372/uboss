...
class ShopcateAction extends CommonAction {
    ...
    private $create_fields = array('cate_name','d1','d2','d3','title', 'orderby','icon');
    private $edit_fields = array('cate_name','d1','d2','d3','title', 'orderby','icon');
    ...
    public function create($parent_id=0) {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->baoSuccess('添加成功', U('bussiness/classify'));
            }
            $this->baoError('操作失败！');
        } else {
    ...
    public function hots($cate_id){
        if ($cate_id = (int) $cate_id) {
            ...
            $obj->cleanCache();
            $this->baoSuccess('操作成功', U('bussiness/classify'));
        } else {
            $this->baoError('请选择要编辑的商家分类');
        }
    }
    ...
    public function delete($cate_id = 0) {
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->baoSuccess('删除成功！', U('bussiness/classify'));
        } else {
    ...
    public function update() {
        ...
        $obj->cleanCache();
        $this->baoSuccess('更新成功', U('bussiness/classify'));
    }
    // 一级分类轮播图
    public function carousel()
    {   
        if(IS_POST){
            $r = M('shop_cate')->save($_POST);
            if($r){
                $this->baoSuccess('添加成功',U('bussiness/classify'));
            }else{
                $this->baoError('添加失败');
            ...
            $carousel = M('shop_cate')->where('cate_id = '.$cate_id)->getField('carousel');

            $carousel1 = array_filter(explode(',',$carousel));
            $this->assign('carousel',$carousel);
            $this->assign('carousel1',$carousel1);
            $this->display();
        }
    ...
    public function ajax_del()
    {
        ...
        $key = $this->_param('key');
        $res = M('shop_cate')->find($cate_id);
        $arr = explode(',',$res['carousel']);
        // 过滤
        $arr = array_filter($arr);
        unset($arr[$key]);
        $img = join(',',$arr);
        $data['carousel'] = $img;
        M('shop_cate')->where('cate_id = '.$cate_id)->save($data);
    }
}