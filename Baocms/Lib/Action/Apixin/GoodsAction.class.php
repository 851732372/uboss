...
//                    ->order('views desc')
    ...
    private function goodsDetail($id)
    {
        ...
        $goodsInfo = $goodsModel
            ->field('g.goods_id id,g.shop_id shopId,g.title name,g.`mall_price`/100 price,g.photos img,g.photo picture,s.shop_name shopName,s.lat,s.lng,b.business_name position,g.details,g.instructions label,g.standard,g.details,s.proportions')
            ->join('uboss_shop s ON s.shop_id = g.shop_id')
            ->join('uboss_business b ON b.business_id = g.business_id')
            ->where("g.goods_id = {$id}")
            ->find();
        $goodsInfo['img'] = array_filter(explode(',', $goodsInfo['img']));
