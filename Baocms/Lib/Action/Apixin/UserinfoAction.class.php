...
class UserinfoAction extends BaseAction
{
    ...
    private $type = array(1 => '充值', 2 => '余额支付', 22 => '扫码支付', 3 => '提现', 4 => 'U店分红', 5 => '消费分成', 6 => '资产变现');
...
//                        ->find();
        ...
        else
        {
            if (!empty($invite_id))
            {
                ...
                {
                    $this->usersModel->save(array('user_id' => $info['user_id'], 'invite_id' => $invite_id, 'pid' => $invite_id));
                }
            }
        }
//        return outJson($info);
...
//                        $confirmPwd = htmlspecialchars($params['confirmPwd']);
     ...
     * 用户余额变动记录
     * @author Ginger
     * @param $type 1 充值 2 余额支付 22 扫码支付 3 提现 4 U店分红 5 消费分成 6 资产变现
     * return
     */
...
//            $where['status'] = 2;
    ...
    private function moneyInfo()
    {
        if ($this->isPost())
        {
            ...
            $usersModel = D('Users');
            $where['user_id'] = $userId;
            $userInfo = $usersModel->field('money')->find($userId);
