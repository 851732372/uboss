...
{
    protected $type = array(1 => '充值', 2 => '变现', 3 => '购买会员', 4 => '申请创始人/股东', 5 => '支付', 6 => '资产退款');
    // 订单状态
     ...
     * 资产充值记录
     * @author Ginger
     * @param $type 1 充值 2 变现 3 购买会员 4 申请创始人/股东 5 支付 6 资产退款
     * return
     */
...
//            $where['order_id'] = array('IN', join(',', $orderId));
            ...
            $income = $moneyLogsModel->where(array('type' => array(array('eq', 5), array('eq', 6), 'OR'), 'user_id' => $userId))->sum('money');
            $data = array(
...
//        $where['ua.user_id'] = $userId;
        ...
        $list = $orderModel
            ->field('o.order_id id,g.title commodityName,g.photo picture,o.status,o.`total_price` paymentMoney,um.`money` profit')
//            ->join('uboss_user_asset_logs ua ON ua.order_id = o.order_id')应该查余额变动表
            ->join('uboss_user_money_logs um ON um.order_id = o.order_id')
    ...
    private function moreData()
    {
        if ($this->isPost())
        {
            ...
            $moneyLogsModel = D('Usermoneylogs');
            $where['user_id'] = $userId;
            $where['type'] = array(array('eq', 5), array('eq', 6), 'OR');
            $where['status'] = 1;
            if ($time == 'today')
            {
                //今日开始、结束时间
                ...
                $todayData = $moneyLogsModel->field('money,status,create_time,ok_time')->where($where)->select();
                $data = array();
                foreach ($todayData as $key => $item)
                {
                    if ($item['status'] == 1)
                    {
...
//        if (($item[$timeType] >= $beginLastThirtyDays + (5 * $fiveDays)) && ($item[$timeType] < $beginLastThirtyDays + (6 * $fiveDays)))
    ...
    public function pieChart()
    {
        ...
        $consumption = $paymentLogsModel->where(array( 'type' => array(array('eq','goods'),array('eq','scan'),'OR')))->where(array('is_paid' => 1))->sum('need_pay');
        $profit = $moneyLogsModel->where(array('type' => 5, 'status' => 2))->sum('money');
        $assetCash = $moneyLogsModel->where(array('type' => 6, 'status' => 2))->sum('money');
        $asset = D('Users')->where(array('user_id' => $userId))->getField('asset');
        $data[] = array(
            'amount' => $consumption,
            'memo' => 'consumption' // 消费
        );
