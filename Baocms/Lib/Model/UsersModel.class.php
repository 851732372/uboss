...
class UsersModel extends CommonModel{
    ...
    public function getUserByUserId($userId){
        $data = $this->find(array('where'=>array('user_id'=>$userId)));
   ...
   public function addIntegral($user_id,$num,$intro=''){
    ...
    {
        $usersModel = D('Users');
        $userInfo = $usersModel->field('pid')->find($userId);
        return $usersModel->field('user_id,level_id,pid,money,asset')->find($userInfo['pid']);
    }

    public function getInviteNum($userId)
    {
        return $this->where(array('pid' => $userId))->count('user_id');
    }

    public function getTeamNum($userId)
    {
        $userIds = $this->field('user_id')->where(array('pid' => $userId))->select();
        $ids = '';
        ...
        $ids = rtrim($ids, ',');
        return D('Apply')->where(array('user_id' => array('IN', $ids), 'status' => 2))->count('apply_id');
    }
}