...
class PaymentModel extends CommonModel {
    ...
    protected $types = array('goods' => '商城购物', 'gold' => '金块充值', 'tuan' => '生活购物', 'money' => '余额充值', 'ele' => '在线订餐', 'ding' => '在线订座', 'assetRecharge' => '资产充值', 'moneyRecharge' => '余额充值', 'buyMember' => '购买会员', 'founder' =>'申请创始人', 'zongjian' =>'申请总监', 'jingli' =>'申请经理', 'zhuguan' =>'申请主管' );
    protected $type = null;
    ...
    public function logsPaid($logs_id) {
        ...
        if (!empty($logs) && !$logs['is_paid']) {
            ...
            if ( D('Paymentlogs') -> save($data)) {//总之 先更新 然后再处理逻辑  这里保障并发是安全的
                ...
                if ($logs['type'] == 'gold') {
                ...
                }elseif ($logs['type'] == 'founder') {  //申请创始人
                    $member =  $usersModel-> find($logs['user_id']);
                    $assetModel = D('UserAssetLogs');
                    $data = array(
                        'user_id' => $logs['user_id'],
                    ...
                    $money = $member['asset'] + $logs['need_pay'];
                    $assetModel->startTrans();
                    if ($assetModel->add($data))
                    {
                        if ($usersModel->where(array('user_id' => $member['user_id']))->setField(array('asset', $money, 'level_id' => 3)))
                        {
                ...
                }elseif ($logs['type'] == 'zongjian' || $logs['type'] == 'jingli' || $logs['type'] == 'zhuguan') {//申请股东
                    $member =  $usersModel-> find($logs['user_id']);
                    $assetModel = D('UserAssetLogs');
                    $data = array(
                        'user_id' => $logs['user_id'],
                    ...
                    if ($assetModel->add($data))
                    {
                        ...
                        if ($usersModel->where(array('user_id' => $member['user_id']))->setField(array('asset' => $money, 'level_id' => $levelId)))
                        {
                            $assetModel->commit();
                        }
                        else
...
//                        'pay_no'
                    ...
                    {
                        if ($usersModel->where(array('user_id' => $member['user_id']))->setInc('money', $logs['need_pay']))
                        {
                            $moneyModel->commit();
                        }
                        else
...
//						$this -> mallSold($logs['order_id']);
                        if ($logs['code'] == 'alipay') $payType = 1;
                        if ($logs['code'] == 'weixin') $payType = 2;
                        ...
                        elseif ($logs['type'] == 'scan')
                        {
                            D('Order') -> save(array('order_id' => $logs['order_id'], 'status' => 8, 'pay_time' => NOW_TIME, 'trade_style' => $payType, 'success_time' => NOW_TIME));
                        }
...
//					D('Tongji') -> log(2, $logs['need_pay']);
    ...
    public function commission($orderId)
    {
        ...
        {
            $shopMoney = $order['original_price'] - $proprotion;
            $shopData = array(
                'shop_id' => $order['shop_id'],
                'money' => $shopMoney,//商家结算
                'create_time' => NOW_TIME,
                'create_ip' => get_client_ip(),
                'intro' => "订单{$orderId}金额{$shopMoney}元",
                'status' => 2,
                'type' => 2,//
                'audit' => 0,
                'order_id' =>$orderId
