...
class Jpush {
	...
	* @param string $param
	*/
	private $_appkeys = 'dd22d94472ae965a398a3573';
	private $_masterSecret = '09cd2d8b0093709ac43976c8';
    ...
    public function __construct() {
        ...
        if($ret){
			...
			$this->_appkeys = $ret['appkey'];
	        $this->_masterSecret = $ret['secret'];
        }else{
        	$this->_appkeys ='';
        	$this->_masterSecret ='';
        }
	...
	*/
	function send($data,$msg_type = 1) {
		if(empty($this->_masterSecret)){
			return false;
		}
		$sendno = $data['sendno'];
		$receiver_type = $data['receiver_type'];
		if($data['uid']){
			$receiver_value = $data['uid'];
			$receiver_type = '3';
		}else{
			$receiver_value = '';
		}
		$msg_content = $data['contents'];
