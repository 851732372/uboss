...
class AopClient {
	...
	protected function sign($data, $signType = "RSA") {
		...
		if ("RSA2" == $signType) {
			$a=openssl_sign($data, $sign, $res, OPENSSL_ALGO_SHA256);
//            var_dump($a);die();
		} else {
...
//print_r($sysParams);exit();
		//系统参数放入GET请求串
		$requestUrl = $this->gatewayUrl . "?";
        $url = '';
		foreach ($sysParams as $sysParamKey => $sysParamValue) {
		    //TODO $requestUrl .= "$sysParamKey=" . urlencode($this->characet($sysParamValue, $this->postCharset)) . "&";
			$url .= "$sysParamKey=" . $this->characet($sysParamValue, $this->postCharset) . "&";
		}
        $url = substr($url, 0, -1);
